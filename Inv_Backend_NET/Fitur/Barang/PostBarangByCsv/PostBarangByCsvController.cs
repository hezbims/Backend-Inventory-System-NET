...
using Inventory_Backend_NET.Fitur._Logic.UseCases;
...
namespace Inventory_Backend_NET.Fitur.Barang.PostBarangByCsv;
...
public class PostBarangByCsvController : ControllerBase
{
        ...
        try
        {
            ...
            using (var csvReader = MyCsvReader.From(streamReader))
            {
                ...
                foreach (var (barangRow , index) in rows.Select((value , i) => (value , i)))
                {
                    ...
                    {
                        targetKategori = new Database.Models.Kategori(nama: barangRow.NamaKategori!);
                    }
                    var newBarang = new Database.Models.Barang(
    ...
    private bool CekNama_KodeBarang_And_Rak_Availability(
        Database.Models.Barang newBarang,
        int currentIndex
    )
    {
    ...
    private void ValidateCsvField(List<CsvBarangDto> rows)
    {
        for (var i = 0 ; i < rows.Count ; i++)
        {
            ...
            if (!(rows[i].NomorRak >= 1 && rows[i].NomorRak <= Database.Models.Barang.MaxNomorRak)) 
                errorRow.Add($"Nomor Rak harus di rentang 1-{Database.Models.Barang.MaxNomorRak}");
            
            if (!(rows[i].NomorLaci >= 1 && rows[i].NomorLaci <= Database.Models.Barang.MaxNomorLaci)) 
                errorRow.Add($"Nomor Laci harus di rentang 1-{Database.Models.Barang.MaxNomorLaci}");
                
            if (!(rows[i].NomorKolom >= 1 && rows[i].NomorKolom <= Database.Models.Barang.MaxNomorKolom)) 
                errorRow.Add($"Nomor Kolom harus di rentang 1-{Database.Models.Barang.MaxNomorKolom}");
